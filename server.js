...
const { emiWarning } = require('node:process');
const app = express();
...
let historicoSensores = [
    ...
    {id:2, temperatura:25, umidade:40, hora:"11:00"},
    {id:3, temperatura:30, umidade:20, hora:"12:00"}
];
...
app.get('/api/dados/:id', (req,res)=> {
    ...
    if (!dado){
        return res.status(400).json({mensagem:"ID não encontrado, tente novamente!"});
    }
    res.json(dado);
});
...
app.post(`/api/dados`, (req,res) => {
    ...
    if(!temperatura||!umidade||!hora){
        res.status(400).json({mensagem:"Dads incompletos."});
    }
...
app.delete('/api/dados/:id', (req,res) => {
    ...
    if (index === -1){
        return res.status(400).json({mensagem:"Dado não encontrado! Tente de novo!"});
    }
